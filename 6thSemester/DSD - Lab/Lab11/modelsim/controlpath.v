module controller (clk, eqz, start, LdA, LdB, LdP, clrp, decB, done);

  input clk, eqz, start;

  output reg LdA = 0 , LdB  = 0 , LdP  = 0 , clrp  = 0 , decB  = 0 , done  = 0;
  reg [2:0] state;
  ...
  always @(*) begin
    case (state)
      ...
      S4:  begin LdA <= 0; LdB <= 0; LdP <= 0; clrp <= 1; decB <= 0; done <= 1; end
    endcase
  end
